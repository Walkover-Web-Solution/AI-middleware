...
const getAllBridges = async (org_id) => {
    try {
        const bridges = await configurationModel.find({ org_id: org_id }, { bridge_id: 1, _id: 1, name: 1, service: 1, org_id: 1, bridgeType: 1, "configuration.model": 1, "configuration.prompt": 1, "configuration.input": 1 });
...
const updateBridgeType = async (bridge_id, org_id, bridgeType) => {
    try {
        ...
        console.log("error:", error);
        return { success: false, error: "something went wrong in updating bridge type!!" };
    }
}
...
module.exports = {
    createBridges, getAllBridges, getBridges, updateBridges, getBridgesByName, deleteBridge, updateToolsCalls, getApiCallById, getBridgesWithSelectedData, addResponseIdinBridge, removeResponseIdinBridge, getBridgeBySlugname, findChatbotOfBridge,
    updateBridgeType
}
