...
/**
 * Schema for GET /:agent_id/:thread_id/:sub_thread_id - getOrchestratorConversationLogs
 * Validates URL params and query params
 */
const getOrchestratorConversationLogs = {
...
/**
 * Schema for GET /orchestrator-threads/:agent_id - getRecentOrchestratorThreads
 * Validates URL params and query params
 */
const getRecentOrchestratorThreads = {
  params: Joi.object().keys({
    agent_id: Joi.string().pattern(/^[0-9a-fA-F]{24}$/).required().messages({
      'string.pattern.base': 'agent_id must be a valid MongoDB ObjectId',
      'any.required': 'agent_id is required'
