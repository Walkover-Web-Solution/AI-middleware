...
const validPromptIds = ['optimze_prompt', 'gpt_memory', 'structured_output_optimizer', 'chatbot_suggestions', 'generate_summary', 'generate_test_cases'];
...
const updatePrebuiltPrompt = {
    body: Joi.object().unknown(true).custom((value, helpers) => {
        ...
        if (!validPromptIds.includes(prompt_id)) {
            return helpers.error('any.custom', { message: `Invalid prompt_id. Must be one of: ${validPromptIds.join(', ')}` });
        }
...
const resetPrebuiltPrompts = {
    body: Joi.object().keys({
        prompt_id: Joi.string().valid(...validPromptIds).required().messages({
...
const getSpecificPrebuiltPrompt = {
    params: Joi.object().keys({
        prompt_key: Joi.string().valid(...validPromptIds).required().messages({
