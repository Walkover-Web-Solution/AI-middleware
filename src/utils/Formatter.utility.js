  ...
  switch (node.type) {
    ...
    case "Card": {
      ...
      if (typeof node.padding === "number") {
        ...
        paddingClasses = node.size === "sm" ? "p-4" : node.size === "lg" ? "p-7" : "p-6";
      }
...
`.trim();
    ...
    case "Row": {
      ...
      const bgClass = node.background && !node.background.startsWith("#") ? node.background : "";

      const borderClass = node.border?.bottom ? "border-b border-slate-200" : "";
...
`.trim();
    ...
    case "Box": {
      ...
      const bgClass = node.background && !String(node.background).startsWith("#") ? node.background : "";

      const bgStyle = node.background && String(node.background).startsWith("#") ? `background:${node.background};` : "";
...
`.trim();
    ...
    case "Title": {
      ...
      const weightClass = node.weight === "semibold" ? "font-semibold" : node.weight === "normal" ? "font-normal" : "font-semibold";
