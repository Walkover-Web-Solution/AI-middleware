  ...
  try {
    ...
    if (status === "published" || status === "updated") {
      ...
      if (body) {
        ...
        axiosCode = `return axios({url:'http://prod-flow-vm.viasocket.com/func/${id}',method:'post',data:data,  headers: {'content-type': 'application/json' } }).then((response) => { return response; }).catch((error) => { return error.response; })`
      }
      else {
        axiosCode = `return axios({url:'http://prod-flow-vm.viasocket.com/func/${id}',method:'get',  headers: {'content-type': 'application/json' } }).then((response) => { return response; }).catch((error) => { return error.response; })`
      }
