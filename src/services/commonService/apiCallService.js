...
const { getAndUpdate } = require("../../services/commonService/configServices").default;

const createsApi = async (req, res) => {
    try {
        const { id, payload, url, desc, status, org_id } = req.body;
        const { bridge_id } = req.params;
        const endpoint = id;
        if (!desc || !id || !status || !bridge_id || !org_id) {
            return res.status(400).json({ error: "Required details must not be empty!!", success: false });
        }
        let axiosCode = ""
        if (status === "published" || status === "updated") {
            ...
            const response = await saveAPI(desc,
                ...
                true)
            if (!response.success) {
                return res.status(400).json({ message: "something went wrong!", success: false, response });
            }
            ...
            if (result.success) {
                return res.status(200).json({ message: "API saved successfully", success: true, activated: true, tools_call: result.tools_call });
            }
            return res.status(400).json(result);
        }
        return res.status(400).json({ message: "Api is not published", success: false });
    } catch (error) {
