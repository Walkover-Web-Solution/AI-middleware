...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      ...
      console.log(
        `Rows with bridge_id: ${bridgeCheck[0].bridge_count}, Rows with version_id: ${versionCheck[0].version_count}`
      );

      // Early exit if no data found
      if (bridgeCheck[0].bridge_count === 0 && versionCheck[0].version_count === 0) {
        console.log(
          "WARNING: No bridge_id or version_id found in conversations table. All bridges will be candidates for deletion."
        );
      }
      ...
      console.log(
        `Found ${usedBridgeIds.size} unique bridge_ids and ${usedVersionIds.size} unique version_ids in conversations table`
      );
      ...
      const activeBridges = await configurationModel
        .find({ status: 1 }, { _id: 1, status: 1, connected_agents: 1 })
        .lean();
      ...
      const allVersions = await versionModel
        .find({ parent_id: { $in: allParentBridgeIds } }, { _id: 1, parent_id: 1 })
        .lean();
      ...
      for (const bridge of allBridges) {
        ...
        if (!isProtected && hasStatusZero) {
          ...
          console.log(
            `Skipping bridge ${bridgeId} because it's already active (status ${bridge.status}) and protected`
          );
        }
      }
        ...
        if (!hasHistory) {
          if (!parentBridgeId) {
            ...
            if (parentBridgeBeingDeleted) {
              ...
              versionsToDelete.push(version._id);
              console.log(
                `Will delete version ${versionId} (parent bridge ${parentBridgeId} is archived and not protected)`
              );
            } else {
              console.log(
                `Skipping version ${versionId} because parent bridge ${parentBridgeId} is protected or active`
              );
            }
          }
        } else {
      ...
      console.log(
        `Identified ${bridgesToUnarchive.length} bridges for unarchiving (based on history and connected agent dependencies)`
      );
      ...
      if (versionsToDelete.length > 0) {
        ...
        for (const apikey of apikeysToUpdate) {
          ...
          if (updatedVersionIds.length !== apikey.version_ids.length) {
            ...
            updatedApikeys++;
            console.log(
              `Updated apikey ${apikey._id} (removed ${apikey.version_ids.length - updatedVersionIds.length} deleted version IDs)`
            );
          }
        }
      ...
      if (bridgesToDelete.length > 0 || versionsToDelete.length > 0) {
        ...
        for (const apiCall of apiCallsToUpdate) {
          ...
          if (updatedBridgeIds.length === 0 && updatedVersionIds.length === 0) {
            ...
            console.log(
              `Updated apicall ${apiCall._id} (removed ${removedBridges} bridge IDs and ${removedVersions} version IDs)`
            );
          }
        }

        console.log(
          `Updated ${updatedApiCalls} apicall documents and deleted ${deletedApiCalls} empty apicall documents`
        );
      }
      ...
      console.log(
        `Summary: Unarchived ${unarchivedBridges} bridges (based on history and connected agent dependencies)`
      );
      console.log(`Summary: Updated ${updatedApikeys} apikey documents (removed deleted version IDs)`);
      console.log(
        `Summary: Updated ${updatedApiCalls} apicall documents and deleted ${deletedApiCalls} empty apicall documents`
      );
    } catch (error) {
