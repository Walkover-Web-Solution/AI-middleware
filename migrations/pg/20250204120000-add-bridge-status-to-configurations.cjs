  ...
  async up() {
    ...
    try {
      ...
      if (apiCallsNeedingNormalization === 0) {
        ...
        const normalizationResult = await apiCalls.updateMany(
          bridgeIdsNullFilter,
          { $set: { bridge_ids: [] } }
        );
        console.log(`Matched ${normalizationResult.matchedCount} apicall documents; normalized ${normalizationResult.modifiedCount} bridge_ids arrays`);
      }
    } catch (error) {
