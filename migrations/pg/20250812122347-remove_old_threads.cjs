      ...
      for (const thread of allThreads) {
        try {
          ...
          if (recordExists) {
            ...
            console.log(
              `Keeping thread ${thread_id} (org: ${org_id}${bridge_id ? `, bridge: ${bridge_id}` : ""}${sub_thread_id ? `, sub: ${sub_thread_id}` : ""}) - found in PostgreSQL`
            );
            keptCount++;
          } else {
            ...
            console.log(
              `Deleted thread ${thread_id} (org: ${org_id}${bridge_id ? `, bridge: ${bridge_id}` : ""}${sub_thread_id ? `, sub: ${sub_thread_id}` : ""}) - not found in PostgreSQL`
            );
            deletedCount++;
          }
        } catch (threadError) {
